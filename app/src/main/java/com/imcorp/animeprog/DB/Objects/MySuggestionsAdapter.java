...
public class MySuggestionsAdapter extends CursorAdapter implements View.OnClickListener {
    ...
    public MySuggestionsAdapter(Context context, SearchView searchView, SearchableInfo searchable,
                                WeakHashMap<String, Drawable.ConstantState> outsideDrawablesCache) {
        super(context, null /* no initial cursor */, true);
        mSearchView = searchView;
        mSearchable = searchable;

        TypedValue typedValue = new TypedValue();
        mContext.getTheme().resolveAttribute(R.styleable.SearchView_commitIcon, typedValue, true);
        //mCommitIconResId = searchView.getSuggestionCommitIconResId();
        mCommitIconResId = typedValue.resourceId;
    ...
    @Override public Cursor runQueryOnBackgroundThread(CharSequence constraint) {
        if(Config.NEED_LOG) Log.d(LOG_TAG, "runQueryOnBackgroundThread(" + constraint + ")");
        ...
        try {
            ...
            if(Config.NEED_LOG) Log.w(LOG_TAG, "Search suggestions query threw an exception.", e);
        }
    ...
    public void close() {
        if(Config.NEED_LOG) Log.d(LOG_TAG, "close()");
        changeCursor(null);
        mClosed = true;
    }
    @Override public void notifyDataSetChanged() {
        if(Config.NEED_LOG) Log.d(LOG_TAG, "notifyDataSetChanged");
        super.notifyDataSetChanged();

        updateSpinnerState(getCursor());
    }
    @Override public void notifyDataSetInvalidated() {
        if(Config.NEED_LOG) Log.d(LOG_TAG, "notifyDataSetInvalidated");
    ...
    @Override public void changeCursor(Cursor c) {
        if(Config.NEED_LOG) Log.d(LOG_TAG, "changeCursor(" + c + ")");

        if (mClosed) {
            if( Config.NEED_LOG )Log.w(LOG_TAG, "Tried to change cursor after adapter was closed.");
            if (c != null) c.close();
            return;
        }
        ...
        try {
            ...
            if(Config.NEED_LOG)Log.e(LOG_TAG, "error changing cursor and caching columns", e);
        }
    }
    /**
    ...
    @Override public View newView(Context context, Cursor cursor, ViewGroup parent) {
        final View v = LayoutInflater.from(context).inflate(R.layout.abc_search_dropdown_item_icons_2line,parent,false);
        v.setTag(new MySuggestionsAdapter.ChildViewCache(v));
    ...
    @Override public void bindView(View view, Context context, Cursor cursor) {
        MySuggestionsAdapter.ChildViewCache views = (MySuggestionsAdapter.ChildViewCache) view.getTag();
        final boolean isAfterLast=cursor.isAfterLast();
        int flags = 0;
        if(isAfterLast){
            view.setOnClickListener(v->{
                SearchProvider.Companion.clear(mContext);
            ...
            //view.setBackgroundResource(R.style.SelectableItemBackground);
        }
        if (mFlagsCol != INVALID_INDEX) {
            flags = cursor.getInt(mFlagsCol);
        }
        if (views.mText1 != null) {
            String text1 = !isAfterLast?getStringOrNull(cursor, mText1Col):mContext.getString(R.string.remove_all_items);
        ...
        if (views.mIcon1 != null) {
            ...
            else setViewDrawable(views.mIcon1, ResourcesCompat.getDrawable(mContext.getResources(), R.drawable.ic_close,null), View.INVISIBLE);
        }
    ...
    @Override public void onClick(View v) {
        ...
        if (tag instanceof CharSequence) {
            mSearchView.setQuery((CharSequence) tag,true);
        }
    }
    ...
    @Override public CharSequence convertToString(Cursor cursor) {
        ...
        String query =cursor.isAfterLast()?mContext.getString(R.string.delete_history): getColumnString(cursor, SearchManager.SUGGEST_COLUMN_QUERY);
        if (query != null) {
            return query;
        }
    ...
    @Override public View getView(int position, View convertView, ViewGroup parent) {
        try {
            ...
            if(Config.NEED_LOG) Log.w(LOG_TAG, "Search suggestions cursor threw exception.", e);
    ...
    private Drawable getDrawableFromResourceValue(String drawableId) {
        ...
        try {
            ...
            if(Config.NEED_LOG)  Log.w(LOG_TAG, "Icon resource not found: " + drawableId);
            return null;
        }
    }
    /**
            ...
            if (ContentResolver.SCHEME_ANDROID_RESOURCE.equals(scheme)) {
                ...
                try {
                    ...
                    try {
                        ...
                        if(Config.NEED_LOG)          Log.e(LOG_TAG, "Error closing icon stream for " + uri, ex);
                    }
                }
            }
        } catch (FileNotFoundException fnfe) {
            if(Config.NEED_LOG) Log.w(LOG_TAG, "Icon not found: " + uri + ", " + fnfe.getMessage());
            return null;
        }
    }
    private Drawable checkIconCache(String resourceUri) {
        ...
        if(Config.NEED_LOG) Log.d(LOG_TAG, "Found icon in cache: " + resourceUri);
    ...
    private Drawable getActivityIcon(ComponentName component) {
        ...
        try {
            ...
            if(Config.NEED_LOG) Log.w(LOG_TAG, ex.toString());
            return null;
        }
        ...
        if (drawable == null) {
            if(Config.NEED_LOG) Log.w(LOG_TAG, "Invalid icon resource " + iconId + " for "
    ...
    private static String getStringOrNull(Cursor cursor, int col) {
        ...
        try {
            ...
            if(Config.NEED_LOG) Log.e(LOG_TAG,
    ...
    Cursor getSearchManagerSuggestions(SearchableInfo searchable, String query, int limit) {
        ...
        final Cursor c = mContext.getContentResolver().query(uri, null, selection, selArgs, null);
        return new Cursor(){
            boolean mIsAfterLast=false;
            @Override
            public int getCount() {
                final int co = c.getCount();
                return co!=0?co+1:0;
            }
            ...
            public boolean isAfterLast() {
                return c.isAfterLast() || mIsAfterLast;
            }
